...
/**
 * @package Activation Email
 * @author Hugo Moracchini
 * @copyright Copyright (c)2018 Hugo Moracchini
...
class plgUserActivation_email extends JPlugin {
		 ...
		 if (JRequest::getInt('emailactivation')) {
			...
			if ($user->guest) {
				...
				if (!empty($token) && strlen($token) === 32 && JRequest::getInt($token, 0, 'get') === 1) {
					...
					// save user data
					if ($table->store()) {
						$app->enqueueMessage(JText::_('PLG_ACTIVATION_EMAIL_ACTIVATED'));
					} else {
	...
	public function onAfterStoreUser($new, $isnew, $result, $error) {
		...
		if ($result && !$error) {
			...
			if ($this->sendActivationEmail($user->getProperties(), $activation)) {
				...
				$app = JFactory::getApplication();
				$app->enqueueMessage(JText::_('PLG_ACTIVATION_EMAIL_SENT'));
	...
	private function sendActivationEmail($data, $token) {
		...
		$subject = preg_replace($keys, $post, $this->params->get('subject'));
		$body = preg_replace($keys, $post, $this->params->get('message'));
